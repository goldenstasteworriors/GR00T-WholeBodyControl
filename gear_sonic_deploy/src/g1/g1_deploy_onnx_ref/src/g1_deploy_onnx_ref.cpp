...
class G1Deploy {
  ...
  public:
    ...
    bool GatherRobotStateToLogger() {
      ...
      auto left_hand_state_ptr = inspire_hands_.getState(true);
      if (left_hand_state_ptr) {
        for (int i = 0; i < 6 && i + 6 < static_cast<int>(left_hand_state_ptr->states().size()); ++i) {
          left_hand_q[i] = left_hand_state_ptr->states()[i + 6].q();
          left_hand_dq[i] = left_hand_state_ptr->states()[i + 6].dq();
        }
      }
      
      auto right_hand_state_ptr = inspire_hands_.getState(false);
      if (right_hand_state_ptr) {
        for (int i = 0; i < 6 && i < static_cast<int>(right_hand_state_ptr->states().size()); ++i) {
